...
appControllers.controller('contractUsCtrl', function ($scope,$http, $cordovaSocialSharing, $cordovaSms,$ionicHistory,$state) {
    ...
    $http.get('app-data/referanslar.json')
...
$http.get("http://185.33.234.241:8080/content/kocEmekInsaat/522d5bc8-607e-4d90-8bb3-3b29c507243c").then(
       function(response){
        $scope.about = response.data.about
        $scope.mission=response.data.mission
        $scope.vision = response.data.vision
        $scope.aboutAuthor = response.data.aboutAuthor
    ...
    $scope.initialForm = function () {
        ...
        $scope.contractInfo = {
            email: "info@kocemek.com",
            telephone:"0541 558 11 51"
